...
var formatDisplayValue = function(value, type, default_value) {
	...
	if(multiply_match) { value = value*parseInt(multiply_match[1]);  type=type.split(multiply_match[0])[0] }

	// float1xx100 or float1 will set fixedPoint to 1.
	var fixed_match    = type.match(/\d+$/);
	var fixed 		   = ((fixed_match)?(function() { type=type.split(fixed_match[0])[0]; return match[0]}):null);
