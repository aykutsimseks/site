...
var create_vis = function() {
	...
	function drawMiniMap() {
		...
		old_visited=["Canada","Austria","Belgium","Bulgaria","Czech Republic","Finland","France","Germany","Greece","Italy","Liechtenstein","Monaco","Netherlands","Russia","Spain","Switzerland","United Kingdom","Vatican City"]
		...
		d3.json("/static/projects/altug_firarda/data/countries.topojson", function(error, world) {
			...
			// Draw maps
			drawMap("#map_canvas1", { 'index': 1, 'color' : 'rgba(51, 122, 183,1)', 'old_color' : 'rgba(	177, 207, 233,1)'})
			$('#map-panel').hide();
		})
	}
	drawMiniMap()
...
function drawMap(elem_id, opts) {
	...
	var world_map = g
		...
		.attr('style', function(d,i){
			var style_json = {
					'cursor' 			: 'pointer',
					'fill'	 			: (visited.contains(d.properties.name))?opts['color']:(old_visited.contains(d.properties.name)?opts['old_color']:"#f0f0f0"),
					'stroke' 			: '#444',
					'stroke-width'		: '.4px',
			...
			if(old_visited.contains(d.properties.name)) { var index = old_visited.indexOf(d.properties.name);old_visited.splice(index, 1);}
